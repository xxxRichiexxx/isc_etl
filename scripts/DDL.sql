...
CREATE TABLE sttgaz.stage_isc_orders (
    ...
    "ModelYear" INT,
    "PriznakRezervirovania" VARCHAR(200),
    "IGC" VARCHAR(500),
    "DirectionOfImplementation" VARCHAR(500),
    "Buyer" VARCHAR(500),
...
CREATE TABLE sttgaz.stage_isc_nomenclature_guide (
    ...
    "CDNuber" VARCHAR(500),
    "Code65" VARCHAR(500),
    "ModelNaZavode" VARCHAR(500),
    "Status" VARCHAR(500),
    "Division" VARCHAR(500),
    "Proizvoditel" VARCHAR(500),
    "VidTovaraID" INT,
    "VidTovara" VARCHAR(500),
    "AutoKit" VARCHAR(500),
    "KisuID" INT,
    "AdabasID" INT,
    "VidProductaID" INT,
    "VidProducta" VARCHAR(500),    
    "load_date" DATE
)
ORDER BY "Code65"
PARTITION BY DATE_TRUNC('month', "load_date");
...
CREATE TABLE sttgaz.dm_isc_sales_RF_CIS (
    ...
    "Дивизион" VARCHAR(50),
    "Дилер" VARCHAR(2000),
    "Дилер. Название из системы скидок" VARCHAR(1000),
    "Территория РФ" VARCHAR(2000),
    "Напр реализ по прилож с учетом УКП" VARCHAR(500),
    "Внутренний код" VARCHAR(500),
    "ВИН" VARCHAR(500),
...
CREATE TABLE sttgaz.dm_isc_contracting(
    ...
    "Прогноз до конца недели" INT,
    "Прогноз до конца месяца" INT
)
