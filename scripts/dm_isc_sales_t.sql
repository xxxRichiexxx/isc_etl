...
("Период", "Дивизион", "Дилер", "Территория продаж", "Внутренний код", "Направление реализации по приложению", "Продажи в розницу", "Продажи физ лицам",
"Остатки на НП", "Остатки на КП", "Продажи в розницу за прошлый месяц", "Продажи физ лицам за прошлый месяц", "Продажи в розницу за прошлый год",
"Продажи физ лицам за прошлый год")
WITH
    dds_data AS(
        SELECT
            d."Дивизион",
	...
	sq4 AS(
		...
		LEFT JOIN dds_data AS s
			...
			AND sq3."Направление реализации по приложению" = s."Направление реализации по приложению"  
		GROUP BY
			DATE_TRUNC('MONTH', sq3."Период")::date,
			sq3."Дивизион",
			sq3."Дилер",
			sq3."Территория продаж",
			sq3."Внутренний код",
		...
		SELECT
			*,
			LAG("Продажи в розницу", 1) OVER (
				PARTITION BY "Дивизион", "Дилер", "Территория продаж", "Внутренний код", "Направление реализации по приложению" ORDER BY "Период"
