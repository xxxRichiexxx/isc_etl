...
WITH
	...
	sq4 AS(
		...
		LEFT JOIN sales_1 																				AS s1_1
			...
			AND HASH(sq3."Дивизион", sq3."Территория продаж", sq3."Внутренний код", sq3."Направление реализации с учетом УКП", sq3."ВИН", sq3."Вариант сборки", sq3."Номерной товар ИД")
				= HASH(s1_1."Дивизион", s1_1."Территория продаж", s1_1."Внутренний код", s1_1."Направление реализации с учетом УКП", s1_1."ВИН", s1_1."Вариант сборки", s1_1."Номерной товар ИД") 
		LEFT JOIN sales_1																				AS s1_2
			...
			AND HASH(sq3."Дивизион", sq3."Территория продаж", sq3."Внутренний код", sq3."Направление реализации с учетом УКП", sq3."ВИН", sq3."Вариант сборки", sq3."Номерной товар ИД")
				= HASH(s1_2."Дивизион", s1_2."Территория продаж", s1_2."Внутренний код", s1_2."Направление реализации с учетом УКП", s1_2."ВИН", s1_2."Вариант сборки", s1_2."Номерной товар ИД") 
		LEFT JOIN sales_2 																				AS s2
			...
			AND HASH(sq3."Дивизион", sq3."Территория продаж", sq3."Внутренний код", sq3."Направление реализации с учетом УКП", sq3."ВИН", sq3."Вариант сборки", sq3."Номерной товар ИД")
				= HASH(s2."Дивизион", s2."Территория продаж", s2."Внутренний код", s2."Направление реализации с учетом УКП", s2."ВИН", s2."Вариант сборки", s2."Номерной товар ИД") 
		LEFT JOIN balance																				AS b
			ON DATE_TRUNC('MONTH', sq3."Дата") = b."Период"
			AND HASH(sq3."Дивизион", sq3."Территория продаж", sq3."Внутренний код", sq3."Направление реализации с учетом УКП", sq3."ВИН", sq3."Вариант сборки", sq3."Номерной товар ИД")
				= HASH(b."Дивизион", b."Территория продаж", b."Внутренний код", b."Направление реализации с учетом УКП", b."ВИН", b."Вариант сборки", b."Номерной товар ИД") 
	),
	sq5 AS(
		SELECT
...
SELECT *
FROM sq5
WHERE "Продажи в розницу" <> 0
	OR "Остатки на НП в пути" <> 0
	OR "Остатки на КП в пути" <> 0
	OR "Остатки на НП" <> 0
	OR "Остатки на КП" <> 0
	OR "Продажи в розницу в прошлом году(по дате продажи)" <> 0
