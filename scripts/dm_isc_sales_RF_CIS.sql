...
("Продажа Дата", "Дивизион", "Дилер", "Территория РФ", "Напр реализ по прилож с учетом УКП", "Внутренний код",
...
WITH
	...
	dds_data AS(
		SELECT
            ...
            s."Остатки на КП",
            s."Период",
			HASH(d."Дивизион", d."Название", s."Территория продаж", s."Внутренний код", s."Направление реализации с учетом УКП", s."Вариант сборки") AS "key"
		FROM sales																	AS s
