...
SELECT
    ...
    AND SUM('Продажи физ лицам') = {{task_instance.pull(key='SoldToIndividuals', task_ids='get_sales')}}
    AND SUM('Остатки на НП') = {{task_instance.pull(key='BalanceAtBeginningOfPeriod', task_ids='get_sales')}}
    AND SUM('Остатки на КП') = {{task_instance.pull(key='BalanceAtEndOfPeriod', task_ids='get_sales')}}
