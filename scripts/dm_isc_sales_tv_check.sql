...
SELECT
    ...
    SUM("Продажи физ лицам") || '; {{task_instance.xcom_pull(key='SoldToIndividuals', task_ids='Загрузка_данных_в_stage_слой.get_sales')}} ||' ||
    SUM("Остатки на НП") || '; {{task_instance.xcom_pull(key='BalanceAtBeginningOfPeriod', task_ids='Загрузка_данных_в_stage_слой.get_sales')}} ||' ||
    SUM("Остатки на КП") || '; {{task_instance.xcom_pull(key='BalanceAtEndOfPeriod', task_ids='Загрузка_данных_в_stage_слой.get_sales')}}',
	'{{execution_date.date()}}',
    SUM("Продажи в розницу") = {{task_instance.xcom_pull(key='SoldAtRetail', task_ids='Загрузка_данных_в_stage_слой.get_sales')}}
