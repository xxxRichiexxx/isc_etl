...
sq3 AS(
	SELECT DISTINCT "Направление реализации с учетом УКП", "Дивизион", "Производитель", "Классификатор подробно по дивизионам 22", "Товар", "ТоварКод65" FROM sq1
),
matrix AS(
	SELECT *
	FROM sq2
...
data AS (
	...
	FROM matrix AS m
	LEFT JOIN sq1 AS s1
		ON HASH(m."Месяц", m."Направление реализации с учетом УКП", m."Дивизион", m."Производитель", m."Классификатор подробно по дивизионам 22", m."Товар", m."ТоварКод65") = 
			HASH(s1."Месяц",s1."Направление реализации с учетом УКП", s1."Дивизион", s1."Производитель", s1."Классификатор подробно по дивизионам 22", s1."Товар", s1."ТоварКод65")
	LEFT JOIN sq1 AS s2
		ON m."Месяц" = s2."Месяц" + INTERVAL '1 YEAR'
		AND HASH(m."Направление реализации с учетом УКП", m."Дивизион", m."Производитель", m."Классификатор подробно по дивизионам 22", m."Товар", m."ТоварКод65") = 
			HASH(s2."Направление реализации с учетом УКП", s2."Дивизион", s2."Производитель", s2."Классификатор подробно по дивизионам 22", s2."Товар", s2."ТоварКод65")
),
windows AS(
SELECT
	*,
	SUM("Реализовано") OVER (
		PARTITION BY DATE_TRUNC('YEAR', "Месяц"), "Направление реализации с учетом УКП", "Дивизион", "Производитель", "Классификатор подробно по дивизионам 22", "Товар", "ТоварКод65"  ORDER BY "Месяц"
