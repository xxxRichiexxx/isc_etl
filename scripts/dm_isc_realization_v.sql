...
sq2 AS(
	...
	FROM (SELECT (DATE_TRUNC('year', '{{execution_date}}'::date) -  INTERVAL '2 YEAR') as tm 
		  UNION ALL
		  SELECT '{{execution_date}}'::date) as t
...
data AS (
	...
	FROM matrix AS m
	LEFT JOIN sq1 AS s1
		ON HASH(m."Месяц", m."Направление реализации с учетом УКП", m."Дивизион",
				m."Производитель", m."Классификатор подробно по дивизионам 22", m."Товар", m."ТоварКод65") = 
			HASH(s1."Месяц",s1."Направление реализации с учетом УКП", s1."Дивизион",
				 s1."Производитель", s1."Классификатор подробно по дивизионам 22", s1."Товар", s1."ТоварКод65")
	LEFT JOIN sq1 AS s2
		ON m."Месяц" = s2."Месяц" + INTERVAL '1 YEAR'
		AND HASH(m."Направление реализации с учетом УКП", m."Дивизион",
				 m."Производитель", m."Классификатор подробно по дивизионам 22", m."Товар", m."ТоварКод65") = 
			HASH(s2."Направление реализации с учетом УКП",s2."Дивизион",
				 s2."Производитель", s2."Классификатор подробно по дивизионам 22", s2."Товар", s2."ТоварКод65")
),
windows AS(
SELECT
	*,
