...
"Напр реализ по прилож с учетом УКП", "Внутренний код", "Вариант сборки", "Розница ТП", "Остаток НП+ВПути",
"Остаток КП+ВПути", "Розница АППГ (по дате продажи)", "Розница АППГ (по дате записи в БД)", 
"Скидка CRM ИТОГО",
"Скидка CRM дилера",
	...
	dds_data AS(
		SELECT
			...
			HASH(d."Дивизион", d."Название", s."Территория продаж", b."Регион", s."Внутренний код",
				s."Направление реализации с учетом УКП", s."Вариант сборки")		AS "key"
		FROM sales																	AS s
