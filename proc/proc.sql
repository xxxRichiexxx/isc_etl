...
--, РабТаб.Собственник         					
  ...
  , СуммаПродажиРозн = sum(case when isnull(РабТаб.ПлощадкаПоставщика,0) in (19,20, 1040 /*ГК СТТ*/) 
								then case when @Валюта = 1 /*руб*/ 
										then РабТаб.Сумма
										else 
											case when isnull(РабТаб.Валюта,1) = @Валюта
...
select
	...
	, Группировка = 23 - 
		grouping(Спр_НомерныеТовары.МодельныйГод)
		 - grouping(НачальнаяВыборка.НомернойТовар)
		 - grouping( isnull(Спр_НоменклатураИсходная.Дивизион,Спр_Номенклатура.Дивизион))
		 - grouping(Спр_НомерныеТовары.Владелец)
		 - grouping(Спр_Площадки_ТерриторияПродаж.СубъектПродаж)
		 - grouping(НачальнаяВыборка.Площадка)
		 - grouping(НачальнаяВыборка.РегионПокупателя
		)
		 - grouping(rtrim(ltrim(isnull(спр_КонтрагентыДилеров.Наименование, НачальнаяВыборка.НаименованиеПокупателя))))
		 - grouping(НачальнаяВыборка.ИннПокупателя)
		 - grouping(Спр_СРМ_Клиенты.ОКВЭД
		)
		 - grouping(case when НачальнаяВыборка.РодЗанятий is not null 
		     then
		         cast(19731/*Спр_ПродажиВРозницу_Анкета_РодЗанятий*/ as bigint) * cast(0x100000000 as bigint) + cast( НачальнаяВыборка.РодЗанятий as bigint)  
		     else
		        cast(19740/*Спр_ПродажиВРозницу_Анкета_СфераДеятельности*/ as bigint) * cast(0x100000000 as bigint) + cast( НачальнаяВыборка.СфераДеятельности  as bigint)  
		 ...
		 - grouping(НачальнаяВыборка.СфераИспользования)
		 - grouping(НачальнаяВыборка.СпецпрограммыРеализацииВСтроку)
		 - grouping(НачальнаяВыборка.ОтгрузкаРМ_Дата)
		 - grouping(НачальнаяВыборка.ПродажаДата)
		 - grouping(НачальнаяВыборка.ПродажаДатаЗаписиВБД)
		 - grouping(НачальнаяВыборка.НаправлениеРеализацииПоПриложению)
		 - grouping(НачальнаяВыборка.НаправлениеРеализацииСУчетомУКП)
		 - grouping(Спр_Площадки.НаправлениеРеализации)
		 - grouping(Спр_НомерныеТовары.ВариантСборкиПродажи)
		 - grouping(Спр_НомерныеТовары.ВариантСборкиФактический_ПоследнееЗначение)
		 - grouping(Спр_Номенклатура.ДвигательПоПрайсу)
		 - grouping(Спр_Заявки.ХолдингКонечныйКлиент)
...
group by
	grouping sets ((), (Спр_НомерныеТовары.МодельныйГод
	,НачальнаяВыборка.НомернойТовар
